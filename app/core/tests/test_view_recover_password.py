...
class RecoverPasswordPostViewTest(TestCase):
    def setUp(self):
        ...
        self.resp = self.client.post(r('recover-password'), {
            'first_name': 'user',
            'last_name': 'test',
            'email': 'user@test.com',
            'password': '!yii2nvbi',
...
class RecoverPasswordInvalidPostViewTest(TestCase):
    def test_first_name(self):
        resp = self.client.post(
            r('recover-password'), self._make_data(first_name=''))
        self.assertContains(resp, 'Este campo é obrigatório.')

    def test_last_name(self):
        resp = self.client.post(
            r('recover-password'), self._make_data(last_name=''))
        self.assertContains(resp, 'Este campo é obrigatório.')

    def test_email(self):
        resp = self.client.post(
            r('recover-password'), self._make_data(email=''))
        self.assertContains(resp, 'Este campo é obrigatório.')

    def test_password(self):
        resp = self.client.post(
            r('recover-password'), self._make_data(password=''))
        self.assertContains(resp, 'Este campo é obrigatório.')
